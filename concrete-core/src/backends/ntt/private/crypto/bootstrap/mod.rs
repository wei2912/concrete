...
pub use buffers::{FourierBskBuffers, NttBuffers};
...
use crate::backends::core::private::math::fft::AlignedVec;
...
impl<Scalar> FourierBootstrapKey<AlignedVec<Scalar>, Scalar>
where
    Scalar: UnsignedTorus,
{
    ...
    /// use concrete_core::backends::core::private::math::fft::Scalar;
    ...
    Scalar: UnsignedTorus,
{
    ...
    /// use concrete_core::backends::core::private::crypto::bootstrap::FourierBootstrapKey;
    /// use concrete_core::backends::core::private::math::fft::Scalar;
    /// let vector = vec![Scalar::new(0., 0.); 256 * 5 * 4 * 4 * 15];
    ...
    /// ```
    pub fn ggsw_iter(&self) -> impl Iterator<Item = GgswCiphertext<&[Scalar]>>
    where
    ...
    ///     ggsw.as_mut_tensor()
    ///         .fill_with_element(Scalar::new(0., 0.));
    /// }
    /// assert!(bsk.as_tensor().iter().all(|a| *a == Scalar::new(0., 0.)));
    /// assert_eq!(bsk.ggsw_iter_mut().count(), 4);
    /// ```
    pub fn ggsw_iter_mut(&mut self) -> impl Iterator<Item = GgswCiphertext<&mut [Scalar]>>
    where
        ...
        Scalar: UnsignedTorus,
    {
        ...
        //output_fft_buffer.fill_with_element(Scalar::new(0., 0.));
        ...
        let mut decomposition = decomposer.decompose_tensor(glwe);
        ...
        loop {
            match (iterator.next(), iterator.next()) {
                (Some(first), None) => {
                    ...
                    ntt.add_backward_as_torus(&mut first_output, &mut first_fourier);
                }
                _ => break,
            }
        }
        ...
        Scalar: UnsignedTorus,
    {
        ...
        self.external_product(ct0, ggsw, ct1, ntt_buffers, rounded_buffer);
    }
