...
///
/// The `tested` argument points to an array of samples which is tested for normality. The `expected_means`
/// argument encodes the mean expected for each element of the `tested` array (can be different for each
/// element). The `expected_variance` argument encodes the variance expected for the whole sample array.
#[allow(dead_code)]
