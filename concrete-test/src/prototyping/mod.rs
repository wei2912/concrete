...
//!
//! Generating the entities used as inputs to engine methods, usually requires several steps to ensure the
//! compatibility on inputs (same secret keys for instance). Maintaining a separate function to
//! generate every combinations of entity types implemented by `concrete_core` backends is not possible. For
//! this reason, all those manipulations are performed on _prototypical_ entities, before being transformed
//! to actual entities buy the [`crate::synthesizing`] module.
//!
...
//! + Manipulation of prototypical entities (encryption/decryption, key generation, pre/post-test operations, ..)
//!
