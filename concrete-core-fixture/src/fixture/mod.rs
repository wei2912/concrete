...
//! it
//!
//! The central abstraction of the library is the [`Fixture`] trait. This trait defines the logic used to
//! sample / test / benchmark the implementors of any of the `*Engine` traits defined in `concrete-core`. This
//! logic is always the same and, depending on the use of the fixture, boils down to:
//!
...
//!
//! For any given `*Engine` trait, a matching `*Fixture` type is defined, which _generically_ implements
//! [`Fixture`]. Here _generically_ means that the implementation of the [`Fixture`] trait do not fix the
//! `Engine` and `Precision` type parameters, but rather restrict them to a family of types. In particular,
//! the `Engine` generic type parameter must implement the `*Engine` trait in question. This `*Fixture` type
//! can then be used to sample / test / benchmark any implementor of the `*Engine` trait.
//!
//! In particular, once the [`Fixture`] mandatory methods and types are defined, the user can benefit from the
//! default methods [`Harness::sample`], [`Harness::test`] or [`Harness::stress`].
...
///
/// To understand how the differents pieces fit, see how the default methods `sample`, `test`, `stress` and
/// `stress_all` use the associated types and methods.
pub trait Fixture<Precision: IntegerPrecision, Engine: AbstractEngine, RelatedEntities> {
    ...
    /// A type containing prototypical values (such as secret keys) passed around the engine execution.
    type Bypass;
    ...
    ///
    /// The first returned value (the bypass), can be used to store prototypical secret keys used in the
    ...
    /// A method which execute the engine in the pre-execution context, returning the post-execution context.
    fn execute_engine(
        parameters: &Self::Parameters,
        engine: &mut Engine,
    ...
    /// A method which verifies the statistical properties of a sample of engine executions, over multiple
    /// randomly generated raw inputs, over multiple sets of parameters.
    ...
    /// A method which verifies the statistical properties of a sample of engine executions, over multiple
    /// randomly generated raw inputs, for a fixed set of parameters.
    fn stress(
        maker: &mut Maker,
        engine: &mut Engine,
        parameters: &Self::Parameters,
    ...
    /// A method which verifies the statistical properties of a sample of engine execution, for a fixed set
    /// of raw inputs and a fixed set of parameters.
    fn test(
        maker: &mut Maker,
        engine: &mut Engine,
        parameters: &Self::Parameters,
    ...
    /// A method which generate a sample of engine execution, for a fixed set of raw inputs and a fixed set
    /// of parameters.
    fn sample(
        maker: &mut Maker,
        engine: &mut Engine,
